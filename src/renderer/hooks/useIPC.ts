...
import { isElectron } from './ipc/constants';
import { useIpcCore } from './ipc/useIpcCore';
import { useSessionIpc } from './ipc/useSessionIpc';
import { usePermissionQuestionIpc, useSudoPasswordIpc } from './ipc/usePermissionIpc';
import { useWorkdirIpc } from './ipc/useWorkdirIpc';
...
export function useIPC() {
          ...
          case 'trace.step': {
            if (event.payload.step.type === 'thinking' && event.payload.step.status === 'running') {
              ...
              if (pending.length > 0) {
                ...
                // Bind the real stepId so mock stepIds cannot leave stale entries
  ...
  const { respondToPermission, respondToQuestion } = usePermissionQuestionIpc({
    send,
  ...
  const { respondToSudoPassword } = useSudoPasswordIpc({
    send,
    setPendingSudoPassword,
  });

  const { selectFolder, getWorkingDir, changeWorkingDir } = useWorkdirIpc({ invoke });

  const { getMCPServers } = useMcpIpc();
