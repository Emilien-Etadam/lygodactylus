...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-6.0.2-orange" alt="Version" />
...
- **Tool-call guard** — relance automatique quand un modèle local (ex. Qwen) émet un tool call en texte `<tool_call>`/`<function=…>` au lieu d'un appel structuré : voir [docs/qwen-local-reliability.md](docs/qwen-local-reliability.md)
...
- **Interface complète à distance (`/app/`)** — au-delà du chat léger, l'interface React complète du desktop est servie sur `http://hôte:port/app/` (lien « Interface complète » dans le chat léger, même token) : streaming temps réel avec raisonnement et traces d'outils, permissions et questions interactives, commandes slash des plugins, fork/édition de messages, panneau de contexte. Les canaux sensibles restent bloqués côté serveur (allowlist) : pas de lecture des clés API (config expurgée), pas de modification de la configuration, du sandbox, des plugins ni des planifications depuis le réseau. Les panneaux Réglages y sont donc non fonctionnels — c'est voulu.
- **Extension Firefox Lygodactylus Web** — traduction, résumé et prompts sur les pages web via le Chat LAN (installable aussi dans les forks Waterfox, LibreWolf, Floorp, Mullvad, Zen) : voir [extension/README.md](extension/README.md)
- **Extension Thunderbird / Betterbird Lygodactylus Mail** — résumé, traduction, analyse d'e-mails et suggestions/reformulations de réponses via le même Chat LAN et le même token extension : voir [extension-thunderbird/README.md](extension-thunderbird/README.md)
