...
| Version                              | Supported          |
| ------------------------------------ | ------------------ |
| `6.x` (current series, latest `6.0.2`) | Yes                |
...
- The **full remote UI** (`/app/`) exposes the same chat capabilities as the desktop (start sessions, approve tool permissions, answer sudo prompts) to any holder of the Chat LAN token — treat that token as equivalent to desktop chat access. Management channels are blocked server-side by an allowlist (`chat-lan-rpc.ts`): remote clients cannot read API keys (config is redacted), change configuration, toggle the sandbox, or install plugins/schedules.
